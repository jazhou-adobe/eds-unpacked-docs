# Foundation & Personas — Design

## Context

This is the first of four planned sub-projects for a new website:
...
The site teaches Edge Delivery Services (EDS) — architecture, tooling, authoring model, and adoption path — drawing on the author's customer workshop experience in Australia.
...
Four personas, each a first-class track, differentiated by what they need from the content:

1. **Architect** — evaluates EDS's technical architecture, integration points, and how it compares to their current AEM/CMS setup. Success: can assess fit and sketch an adoption/migration architecture.
2. **Developer** — wants the authoring model (document-based authoring), tooling, local dev workflow. Success: comfortable building or contributing to an EDS project.
3. **IT Manager / Operations** — cares about governance, hosting/ops model, security, support model, maintenance overhead. Success: confident EDS fits their operational and compliance requirements.
4. **Business Decision-Maker** — cares about outcomes: content velocity, cost, risk, time-to-market, competitive position. Success: can justify or approve an investment in EDS adoption.

"Equal first-class tracks" means each persona is designed for and tagged from day one — **not** that launch requires equal content volume per persona. Workshop source material skews technical (Architect/Developer); IT Manager and Business content will likely lag initially. This imbalance is an accepted, explicit trade-off, not a launch blocker.
...
- **Persona = metadata, not navigation.** Each piece of content carries one or more persona tags; visitors filter by persona. There are no dedicated persona landing pages or curated journeys in this phase — that's a possible future iteration once there's enough content to sequence.
...
Success is qualitative for this phase: a visitor can articulate what EDS is and why it matters to their organization, can correct at least one common myth about it, and knows where to go next based on persona/topic tags.
...
- Persona-curated paths beyond simple tags — possible future iteration.
